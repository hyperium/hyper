...
async fn conn_close_request_is_not_pooled_even_if_backend_keeps_alive() {
    ...
    assert_eq!(
        keepalive_backend_conn_counts(&[], 2).await,
        vec![1, 1],
    ...
    assert_eq!(
        keepalive_backend_conn_counts(&["close"], 2).await,
        vec![1, 2],
    ...
    assert_eq!(
        keepalive_backend_conn_counts(&["keep-alive, close"], 2).await,
        vec![1, 2],
        "a `close` token in a comma-separated Connection value must not be pooled"
    );
    // A `close` in ANY of multiple Connection header lines is honored (get_all,
    // not just the first line).
    assert_eq!(
        keepalive_backend_conn_counts(&["keep-alive", "close"], 2).await,
        vec![1, 2],
        "a `close` in any Connection header line must not be pooled"
    );
}

// Sends `num_requests` sequential requests (with the given `Connection` header lines,
// one `.header()` append each) through a client whose backend always replies keep-alive,
// and returns the cumulative number of connections opened observed after each request.
async fn keepalive_backend_conn_counts(
    connection_values: &[&'static str],
    num_requests: usize,
) -> Vec<usize> {
    ...
    for i in 0..num_requests {
        ...
        crate::body::to_bytes(res.into_body())
            .await
            .expect("drain body");
