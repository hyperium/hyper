...
fn test_parse_header_no_quality() {
    ...
    let b = Accept(vec![
        parsing::QualityItem{item: mime::Mime(mime::TopLevel::Text, mime::SubLevel::Plain, vec![(mime::Attr::Charset, mime::Value::Utf8)]), quality: 1f32},
    ]);
    assert_eq!(a, b);
}
...
fn test_parse_header_with_quality() {
    ...
    let b = Accept(vec![
        parsing::QualityItem{item: mime::Mime(mime::TopLevel::Text, mime::SubLevel::Plain, vec![(mime::Attr::Charset, mime::Value::Utf8)]), quality: 0.5f32},
    ]);
    assert_eq!(a, b);
}
