...
impl<T: Transport, H: MessageHandler<T>> Conn<T, H> {
    ...
    fn read<F: MessageHandlerFactory<T, Output=H>>(&mut self, scope: &mut Scope<F>, state: State<H, T>) -> State<H, T> {
         match state {
            State::Init => {
    ...
    fn write<F: MessageHandlerFactory<T, Output=H>>(&mut self, scope: &mut Scope<F>, mut state: State<H, T>) -> State<H, T> {
    ...
    where F: MessageHandlerFactory<T, Output=H> {
        trace!("Conn::ready events='{:?}'", events);
...
where F: FnMut(Control) -> H, H: MessageHandler<T>, T: Transport {
    type Output = H;
    fn create(&mut self, ctrl: Control) -> H {
        self(ctrl)
    }
}
