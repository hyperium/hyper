...
ffi_fn! {
    fn hyper_request_set_method(req: *mut Request<Body>, method: *const u8, method_len: size_t) -> hyper_code {
        let bytes = unsafe {
...
ffi_fn! {
    fn hyper_request_set_uri(req: *mut Request<Body>, uri: *const u8, uri_len: size_t) -> hyper_code {
        let bytes = unsafe {
