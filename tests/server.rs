...
fn serve_with_timeout(dur: Option<Duration>) -> Serve {
    ...
    thread::Builder::new().name(thread_name).spawn(move || {
        let handle = Server::http(&"127.0.0.1:0".parse().unwrap()).unwrap()
...
#[test]
fn server_timeout() {
    let server = serve_with_timeout(Some(Duration::from_secs(1)));
    let mut req = TcpStream::connect(server.addr()).unwrap();
    req.write_all(b"\
        POST / HTTP/1.1\r\n\
        Host: example.domain\r\n\
    ...
    req.write_all(b"1234567890").unwrap();
    assert_eq!(req.write_all(b"1").unwrap_err().kind(), io::ErrorKind::BrokenPipe);
}
